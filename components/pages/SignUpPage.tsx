...
const SignUpPage: React.FC<SignUpPageProps> = ({ onSignUp, navigateTo, addToast }) => {
    ...
    <div className="animate-fade-in flex items-center justify-center py-12">
      <div className="w-full max-w-md">
        <div className="bg-surface dark:bg-surface-dark p-8 rounded-2xl shadow-2xl border border-border-color/50 dark:border-border-color-dark/50">
          <h2 className="text-3xl font-bold text-center text-text-primary dark:text-text-primary-dark mb-2">Create an Account</h2>
          <p className="text-center text-text-secondary dark:text-text-secondary-dark mb-8">Join SkyBet today!</p>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="username" className="block text-sm font-medium text-text-secondary dark:text-text-secondary-dark mb-2">
                Username
              </label>
              <input
                type="text"
        ...
        <p className="text-center mt-6 text-text-secondary dark:text-text-secondary-dark">
            Already have an account? <button onClick={() => navigateTo('Login')} className="font-medium text-primary hover:text-primary-dark bg-transparent border-none p-0 disabled:text-slate-400" disabled={isLoading}>Log in here.</button>
        </p>
      </div>
    </div>
  );
