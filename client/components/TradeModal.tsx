...
export const TradeModal = ({ user, stock, type, onClose, onConfirm }: TradeModalProps) => {
  ...
  // ✅ Correction ici pour le nouveau type de portefeuille
  const userShares = user.portfolio?.[stock.ticker]?.quantity ?? 0;
  const availableToBuy = stock.circulatingSupply ?? 0;

  const isBuyDisabled = type === 'buy' && (quantity <= 0 || quantity > availableToBuy || totalValue > (user.cash ?? 0));
  const isSellDisabled = type === 'sell' && (quantity <= 0 || quantity > userShares);
  ...
  const infoValue = type === 'buy'
    ? (user.cash ?? 0).toLocaleString('fr-FR', { style: 'currency', currency: 'USD' })
          ...
          <div className="info-grid">
            <div className="info-item">
              ...
              <span className="info-value">
                {stock.price.toLocaleString('fr-FR', { style: 'currency', currency: 'USD' })}
              </span>
            </div>
            <div className="info-item">
          ...
          <div className="form-group">
            ...
            <div className="quantity-section">
              ...
              <input
                ...
                aria-label="Quantité"
              />
              <button className="btn" onClick={() => setQuantity(q => Math.min(q + 1, type === 'buy' ? availableToBuy : userShares))}>+</button>
            </div>
            {type === 'buy' && (
